...
- **跳页**：在进度栏输入行号回车即可跳转
- **EPUB 支持**：自动解包 epub 正文，左侧目录可点击跳转章节，正文插图等比缩放显示
- **上下滚动**：正文溢出时阅读区自动出现滚动条，随窗口大小自适应
- **编码自动识别**：支持 UTF-8（含 BOM）/ GBK / ANSI，切书自动重新检测
- **后台读取**：翻页、跳页、刷新均不阻塞 IDE
- **离线朗读**：一键朗读小说，读完自动翻下一页续读；可选系统语音、实时调整语速，全程离线（Windows SAPI / macOS `say` / Linux `espeak-ng`）
...
| 跳页 | 在进度栏输入行号后回车 |
| 播放 / 停止朗读 | 点击 `Play` / `Stop` 按钮，或按 `Ctrl+4`（可自定义） |
...
> 朗读语音与语速在阅读窗口顶部的下拉框中直接选择，无需重启，实时生效。
